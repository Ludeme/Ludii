...
public abstract class SpatialFeature extends Feature
{
	...
	public static SpatialFeature combineFeatures(final Game game, final FeatureInstance a, final FeatureInstance b)
	{
		...
		if (bHasRegionProxim)
		{
			...
			for (int i = modifiedPatternB.featureElements().size() - 1; i >= 0; --i)
			{
				if (modifiedPatternB.featureElements().get(i).type() == ElementType.RegionProximity)
					modifiedPatternB.featureElements().remove(i);
			}
		}
