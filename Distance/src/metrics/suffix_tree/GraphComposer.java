...
/**
 * Composes graphs for the programm yED.
 * Currently addapted for suffix tree nodes.
 * 
 * @author Markus
 * @author Zhangyi
 *
...
public class GraphComposer
{
	...
	GMLComposerSettings settings = GMLComposerSettings.getDefaultInstance();

	public GraphComposer(final GMLComposerSettings settings)
	{
	...
	{
		this.settings = GMLComposerSettings.getDefaultInstance();
	}
	...
	{
		testFileName = name + "_" + settings.getName();
		final DateTimeFormatter dtf = DateTimeFormatter
				.ofPattern("MM-dd-HHmmss");
		final LocalDateTime time = LocalDateTime.now();
		final String now = dtf.format(time);
		...
		final String path2 = folder.getAbsolutePath() + "/" + testFileName + "_"
				+ now + extension;
		...
		System.out.println("graph written to " + path2);
//		return anode;
	}
	...
	{
		final String stringID = String.format("\t\t\t id \t %d", Integer.valueOf(id));
		final String stringLabel = String.format("\t\t\t label \t \"%s\"",
				label);
		final String stringx = String.format("\t\t\t\t x \t %.0f", Float.valueOf(x));
		final String stringy = String.format("\t\t\t\t y \t %.0f", Float.valueOf(y));
		final String stringw = String.format("\t\t\t\t w \t %.0f", Float.valueOf(w));
		final String stringh = String.format("\t\t\t\t h \t %.0f", Float.valueOf(h));
	...
	{
		final String stringSource = String.format("\t\tsource\t%d", Integer.valueOf(sourceId));
		final String stringTarget = String.format("\t\ttarget\t%d", Integer.valueOf(targetId));
		...
		if (!targetArrow) stringTargetArrow = "";
	 ...
	 * @param fileName the name the file should be. extension will be added
	 * @param suffixTree 
	 * @param includeSuffixLinks TODO
	 */
	public void compose(final File folder, final String fileName,final SuffixTree suffixTree, final boolean includeSuffixLinks)
	{
		final ArrayList<ArrayList<String>> nAe = extractNodeEdgeInfos(
				suffixTree, includeSuffixLinks);
	...
	/**
	 * used for compose gml graphs
	 * transforms the nodes of 
	 * 
	 * @param suffixTree
	 * @param includeSuffixLinks including of the suffixLinks, but this clutters the tree and is only used during the building of the suffix tree
	 * @return
	 */
	private ArrayList<ArrayList<String>> extractNodeEdgeInfos(
			final SuffixTree suffixTree, final boolean includeSuffixLinks
	)
	{
		...
		System.out.println();
		final ArrayList<Node> allNodes = suffixTree.getAllNodes();
		...
		for (int i = 0; i < allNodes.size(); i++)
		{
			...
			String newNode;
//			
			newNode = composeNode(suffixTree, nodeState,idMap);
			String newEdge;
			
			final Collection<? extends Node> nl = nodeState.getNodeChildren();
			for (final Node childNode : nl)
			{
				newEdge = composeEdge(suffixTree,idMap, nodeState, childNode,true);
				edges.add(newEdge);
			}
			
			final Node suflinkNode = nodeState.getSuflink();
			if (suflinkNode != null&&includeSuffixLinks)
			{
				newEdge = composeEdge(suffixTree,idMap, nodeState,suflinkNode,false);
				edges.add(newEdge);
			}
			nodes.add(newNode);
	...
	private String composeEdge(
			final SuffixTree suffixTree, final HashMap<Node,Integer> idMap, final Node parentNode,
			final Node nodeState, final boolean targetArrow
	)
	{
		final String newEdge = composeEdge(idMap.get(parentNode).intValue(),
				idMap.get(nodeState).intValue(), settings.getEdgeThickness(null),
				settings.getEdgeColor(null), Boolean.valueOf(false), targetArrow);
		return newEdge;
	}


	private String composeNode(
			final SuffixTree suffixTree, final Node nodeState, final HashMap<Node,Integer> idMap
	)
	{
		String newNode;
		final int mover = 0;
		...
		newNode = composeNode(idMap.get(nodeState).intValue(), 12,
				...
				settings.getWidth(nodeState), settings.getHeight(nodeState, label.split("\\n").length),
				"no label",
				settings.getShapeFromMover(mover, nodeState),
				settings.getFillColorFromMover(mover, nodeState, suffixTree),
