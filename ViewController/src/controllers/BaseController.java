	...
	public Location calculateNearestLocation(final Context context, final Point pt, final List<Location> legalLocations)
	{
		...
		for (final Location location : legalLocations)
		{
			try
			{
				...
				final PieceStackType componentStackType = context.metadata().graphics().stackType(container, context, location.site(), location.siteType(),  cs.state(location.site(), location.level(), location.siteType()));
