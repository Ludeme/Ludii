 ...
 * Evaluates a position in the KRK endgame. Adapted from Bolognesi.
 */
public class KRKMainComponent extends EvaluationFunctionComponent {
	...
	public float evaluate(Metaposition start, Metaposition dest, Move m, Vector history)
	{
		...
		int rook = dest.getSquareWithPiece(Chessboard.ROOK);
		rx = rook/8; ry = rook%8;
		...
		int f3 = evalAreas(meta,rx,ry,kx,ky);
		...
		if (canBeIllegal(start,dest,m))
		{
			if (history.size()>0) { result-=10; m.checkmate=true;}
			
		}
		for (int ind=0; ind<history.size(); ind++)
		{
			Move h = (Move)history.get(ind); 
			if (h.checkmate) result-=10; //hack hack hack
		}
	...
	private int evalAreas(Metaposition m, int rx, int ry, int kx, int ky)
	{
		...
		for (int k=0; k<4; k++)
		{
			...
			boolean found = false;
			int x1,x2,y1,y2;
			if (k<2) { x1 = 0; x2 = rx-1;} else { x1 = rx+1; x2 = 7; }
			if (k%2==0) { y1 = 0; y2 = ry-1; } else { y1 = ry+1; y2 = 7; }
		...
		/*if (areas==2)
		{
			...
			if (KRKEvaluationFunction.verbose)
				System.out.println(quad[0]+" "+quad[1]+" "+quad[2]+" "+quad[3]);
			
			if (hor) 
			{
				if (KRKEvaluationFunction.verbose) System.out.println("H!");
				int x = (ry>7-ry?ry:7-ry);
				bonus+=(x*2);
			}
			if (ver) 
			{
				if (KRKEvaluationFunction.verbose) System.out.println("V!");
		...
		if (areas==1)
		{
			int q = (quad[0]? 0 : quad[1]? 1 : quad[2]? 2 : 3);
	...
	private int adjacentRook(int rx, int ry, int kx, int ky)
	{
		...
		if (dist==1) return 10; else return 0;
	}
