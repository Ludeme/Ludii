			...
			for (int j = 0; j < POLICY_WEIGHT_TYPES.length; ++j)
			{
				...
				for (final int depth : TREE_DEPTHS)
				{
					...
					for (int p = 1; p < featureSets.length; ++p)
					{
						...
						final DecisionTreeNode root = ExperienceIQRTreeLearner.buildTree(featureSets[p], linearFunctions[p], buffer, depth);
					...
					final String outFile = RESULTS_DIR + cleanGameName + "_" + cleanRulesetName + "/IQRTree_" + POLICY_WEIGHT_TYPES[j] + "_" + depth + ".txt";
